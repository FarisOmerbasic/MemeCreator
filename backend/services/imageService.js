...
async function preview(imageBuffer, params = {}) {
  const scale = Math.max(
    0.01,
    Math.min(0.25, Number(params.scaleDown || params.scale) || 0.05)
  );
  ...
  if (params.watermarkImageBuffer) {
    const watermarkLayer = getWatermarkPosition(
      params.watermarkImageBuffer,
  ...
  const formatMethod =
    outputFormat === "jpg" || outputFormat === "jpeg" ? "jpeg" : outputFormat;
